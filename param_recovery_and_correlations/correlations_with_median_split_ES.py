...
if ACC_CSV is None:
    raise FileNotFoundError("Couldn't find ES accuracy CSV...")
print(f"Using ES accuracy file: {ACC_CSV}")

# alpha split file (from LE phase)
ALPHA_SPLIT_CSV = OUT_DIR / "alpha_median_split.csv"
if not ALPHA_SPLIT_CSV.exists():
    raise FileNotFoundError("alpha_median_split.csv not found — run LE analysis first!")
...
def _extract_all_subject_params(df, central="mean"):
    ...
    for _, row in df.iterrows():
        ...
        if not m:
            continue
        base, mod, sid = m.group("base"), m.group("mod"), int(m.group("sid"))
...
def add_theta_params_to_results(m35_in_csv, m35_out_csv, use_median=False):
    ...
    num_InatWS, num_InatWE, att = subj_maps[need["num_v_ES_InattentionW_S"]], subj_maps[need["num_v_ES_InattentionW_E"]], subj_maps[need["num_v_ES_AttentionW"]]
    combos = [("theta_InatWS", num_InatWS, att), ("theta_InatWE", num_InatWE, att)]
...
def plot_accuracy_correlations_with_split(
    accuracy_csv: Path,
    model35_results_csv: Path,
    alpha_split_csv: Path,
    out_pdf: Path,
    out_csv: Path,
    out_es_split_csv: Path,
    use_median: bool = False
):
    # accuracy per subject
    acc_map = read_es_accuracy(accuracy_csv)

    # load LE-based alpha split
    split_df = pd.read_csv(alpha_split_csv)
    group_map_le = {int(row["subj"]): row["group"] for _, row in split_df.iterrows()}

    # independent ES-based median split (for later comparison)
    median_es = np.median(list(acc_map.values()))
    group_map_es = {sid: ("poor" if acc <= median_es else "good") for sid, acc in acc_map.items()}
    pd.DataFrame([{"subj": sid, "accuracy": acc, "group_ES": group_map_es[sid]} for sid, acc in acc_map.items()])\
        .to_csv(out_es_split_csv, index=False)
    ...
    for base_name, subj_map in sorted(params_by_name.items()):
        ...
        y = np.array([subj_map[s] for s in common])     # parameter
        groups_le = [group_map_le.get(s, "NA") for s in common]
        ...
        panels.append(dict(name=base_name, x=x, y=y, groups=groups_le,
                           x_line=x_line, y_line=y_line, y_lo=y_lo, y_hi=y_hi,
                           r2=r2, p=p))

        for sid, xi, yi, g in zip(common, x, y, groups_le):
            rows.append({"parameter": base_name, "subj": sid, "accuracy": xi, "param_val": yi,
                         "group_LE": g, "pearson_r": r, "r2": r2, "p_value": p})
    ...
    colors = {"good": "steelblue", "poor": "darkorange", "NA": "gray"}
    for ax, panel in zip(axes, panels):
        for xi, yi, g in zip(panel["x"], panel["y"], panel["groups"]):
            ax.scatter(xi, yi, s=40, color=colors.get(g, "gray"), alpha=0.85, edgecolors="none")
    ...
    fig.suptitle("ES accuracy vs aDDM parameters (colored by LE α median split)", y=0.995)
    ...
    print(f"Saved ES split: {out_es_split_csv}")
...
out_pdf = OUT_DIR / "accuracy_vs_params_with_theta_colored_by_LEsplit.pdf"
out_csv = OUT_DIR / "accuracy_vs_params_with_theta_colored_by_LEsplit.csv"
out_es_split_csv = OUT_DIR / "es_median_split.csv"

plot_accuracy_correlations_with_split(
    accuracy_csv=ACC_CSV,
    model35_results_csv=m35_aug_csv,
    alpha_split_csv=ALPHA_SPLIT_CSV,
    out_pdf=out_pdf,
    out_csv=out_csv,
    out_es_split_csv=out_es_split_csv,
    use_median=False
)
