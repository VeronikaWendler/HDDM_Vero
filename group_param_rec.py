# ============================================================
# Parameter-recovery loop – group-level only
# ============================================================
import os, warnings
...
import numpy as np
import pandas as pd
import arviz as az              # only for reading the *empirical* fit
import hddm
...
def simulate_dataset(true_pars, raw_df):
    """Simulate a full data set from one joint draw of group parameters."""
    sim_rows = []
    for _, tr in raw_df.iterrows():
        ...
        for col in ["subj_idx", "OVcate", "z_AttentionW",
                    "z_IAW_chart", "z_IAW_image"]:
            trial_df[col] = tr[col]
...
def refit_and_get_means(sim_df, seed):
    """Fit the HDDM *once* and return posterior means (dict)."""
    np.random.seed(seed)        # reproducible traces
    ...
    mdl.sample(
        N_SAMPLES,
        burn=BURN,
        db='ram',                 # nothing written to disk
        dbname=f'ram_{seed}',
    )

    # grab posterior means directly from the trace (no ArviZ needed)
...
for rep in trange(N_REPS, desc="parameter-recovery", unit="rep"):
    ...
    θ_hat  = refit_and_get_means(sim_df, seed=10_000 + rep)

    for p in PARAM_LIST:
        records.append(dict(rep=rep, parameter=p,
...
g = sns.FacetGrid(results, col="parameter", col_wrap=3,
                  sharex=False, sharey=False, height=3.0)
...
# N_REPS    = 3         # ≥ 500 recommended for a paper
