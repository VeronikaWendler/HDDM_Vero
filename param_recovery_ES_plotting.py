...
# ------------------ environment tweaks for cluster ------------------
# matplotlib cache writable
cache_dir = os.path.abspath("./.matplotlib_cache")
os.environ["MPLCONFIGDIR"] = cache_dir
os.makedirs(cache_dir, exist_ok=True)
# disable numba jit/caching issues
os.environ["NUMBA_DISABLE_JIT"] = "1"
# dummy modules (if needed)
...
FIG_DIR_ROOT = PROJECT_DIR / "figures_dir_garcia/garcia_replication_ES_29/diagnostics"
FIG_DIR_ROOT.mkdir(parents=True, exist_ok=True)

# ------------------ load inference data ------------------
# Fitted model (original)
...
fitted_idata = az.concat([chain0, chain1, chain2], dim="chain")

# Recovery model
recovered_path = BASE_MODEL_DIR / "mES_29_recovery.nc"
recovered_idata = az.from_netcdf(recovered_path)

# ------------------ diagnostics helper ------------------
def save_diagnostics(idata, label, outdir, var_names=None):
    outdir = Path(outdir)
    outdir.mkdir(parents=True, exist_ok=True)
    summary = az.summary(idata, var_names=var_names, round_to=3)  # includes r_hat, ess_bulk, ess_tail
    ...
    if "r_hat" in summary.columns:
        bad = summary["r_hat"] > 1.05
        if bad.any():
            print(f"[{label}] WARNING: high r_hat for: {list(summary.index[bad])}")
    if "ess_bulk" in summary.columns:
        low = summary["ess_bulk"] < 100
        if low.any():
            print(f"[{label}] WARNING: low bulk ESS for: {list(summary.index[low])}")
    ...
    try:
        ...
        plt.close(trace_fig)
        print(f"[{label}] trace plot saved to {trace_path}")
    except Exception as e:
        print(f"[{label}] could not make trace plot: {e}")
    ...
    try:
        ...
        loo_df = pd.DataFrame({
            "loo": [loo_res.loo],
            "p_loo": [loo_res.p_loo],
            "pareto_k_mean": [np.mean(loo_res.pareto_k.values)]
        })
        loo_path = outdir / f"{label}_loo.csv"
        loo_df.to_csv(loo_path, index=False)
        print(f"[{label}] LOO saved to {loo_path}")
    except Exception as e:
        print(f"[{label}] LOO failed: {e}")

    return summary

# define the parameter list appropriate for current model
# note: 'v_val_bal_int' might appear depending on naming; we include both to be safe
param_list = [
    "t",
    "v_Intercept",
    "v_val_diff",
    "v_val_bal_int",  # orthogonalized balance term (if named slightly differently adjust below)
    "a",             # fallback; HDDM sometimes stores 'a' or 'a_Intercept'
]

# filter to those that actually exist in posterior
def existing_params(idata, candidates):
    return [p for p in candidates if p in idata.posterior.data_vars]

group_param_list = existing_params(fitted_idata, param_list)

# ------------------ save diagnostics ------------------
fitted_summary = save_diagnostics(fitted_idata, "fitted", FIG_DIR_ROOT, var_names=group_param_list)
recovered_summary = save_diagnostics(recovered_idata, "recovered", FIG_DIR_ROOT, var_names=group_param_list)

# ------------------ posterior density comparison ------------------
# decide final common param names for plotting (exclude None)
plot_params = group_param_list.copy()
# Normalize name selection if both a and a_Intercept present, keep only one
...
for i, param in enumerate(plot_params):
    ax = axes[i]
    try:
        az.plot_posterior(fitted_idata.posterior[param],
                          ax=ax,
                          hdi_prob=0.94,
                          point_estimate="mean",
                          label="Fitted",
        ...
        az.plot_posterior(recovered_idata.posterior[param],
                          ax=ax,
                          hdi_prob=0.94,
                          point_estimate="mean",
                          label="Recovered",
        ...
        # remove duplicate legend entries per axis
        if ax.get_legend():
            ax.legend_.remove()
        if i % n_cols != 0:
            ax.set_ylabel("")
...
fig.legend(handles, ["Fitted", "Recovered"], loc="lower center", ncol=2)
plt.tight_layout()
posterior_path = FIG_DIR_ROOT / "posterior_density_comparison.png"
fig.savefig(posterior_path, dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"Saved posterior comparison at {posterior_path}")

# ------------------ forest plot group-level ------------------
fig, ax = plt.subplots(figsize=(8, 6))
az.plot_forest([fitted_idata, recovered_idata],
               ...
               hdi_prob=0.95,
               ax=ax)
ax.set_title("Group-level Parameter Recovery")
forest_group_path = FIG_DIR_ROOT / "forest_group_comparison.png"
plt.tight_layout()
plt.savefig(forest_group_path, dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"Saved group-level forest plot at {forest_group_path}")

# ------------------ individual-level regression recovery plots ------------------
# helper to extract subject-level means from summary tables
def subject_means_from_idata(idata):
    # get summary with subj-specific params (pattern: param_subj.X)
    raw = az.summary(idata, kind="stats", round_to=3)
    # We need per-subject means: use the custom extraction used earlier
    df = raw.reset_index()
    # infer param_name column
    if "param_name" in df.columns:
        name_col = "param_name"
    elif "index" in df.columns:
        name_col = "index"
    else:
        raise RuntimeError("Cannot find parameter name column in summary output")
    # extract subj-specific entries like 'v_val_diff_subj.2' if that naming exists; fallback to hierarchical if not
    pattern = r"(.*)_subj\.(\d+)"
    extract = df[name_col].str.extract(pattern)
    if extract.isna().any().any():
        # no subj-specific, fall back to group-level (will not make regression)
        return None
    df[["param", "subj_idx"]] = df[name_col].str.extract(pattern)
    df = df.dropna(subset=["subj_idx"])
    df["subj_idx"] = df["subj_idx"].astype(int)
    # pivot to have columns per param, index subj_idx, take mean
    values = df.pivot(index="subj_idx", columns="param", values="mean")
    return values

fitted_subjects = subject_means_from_idata(fitted_idata)
recovered_subjects = subject_means_from_idata(recovered_idata)

# If we have subject-level extractable, do regression plots; otherwise skip but warn
if fitted_subjects is not None and recovered_subjects is not None:
    common_subj = fitted_subjects.index.intersection(recovered_subjects.index)
    if len(common_subj) < max(len(fitted_subjects), len(recovered_subjects)):
        print(f"Warning: only {len(common_subj)} common subjects for regression recovery plots")

    fitted_aligned = fitted_subjects.loc[common_subj]
    recovered_aligned = recovered_subjects.loc[common_subj]

    # choose parameters present in both
    reg_params = [p for p in plot_params if p in fitted_aligned.columns and p in recovered_aligned.columns]

    # regression helper
    def regplot_with_corr(x, y, ax, scatter_kws=None, annot=True):
        ...
        x = x[mask]
        y = y[mask]
        sns.regplot(x=x, y=y, ax=ax, scatter_kws=scatter_kws, ci=95)
        # correlation
        if len(x) > 1:
            r, p = pearsonr(x, y)
            ...
            annot_text = f"$r={r:.2f}$\n{p_str}\n$\\beta_0={intercept:.2f}$\n$\\beta_1={slope:.2f}$"
            ax.annotate(annot_text, xy=(0.05, 0.95), xycoords="axes fraction",
                        ha="left", va="top",
    ...
    fig, axes = plt.subplots(ncols=len(reg_params), figsize=(3 * len(reg_params), 3))
    if len(reg_params) == 1:
        axes = [axes]
    for i, param in enumerate(reg_params):
        ax = axes[i]
        regplot_with_corr(fitted_aligned[param], recovered_aligned[param], ax=ax)
        ax.set_title(param)
    plt.tight_layout()
    reg_path = FIG_DIR_ROOT / "regression_recovery_plots.png"
    fig.savefig(reg_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved regression recovery plots at {reg_path}")
else:
    print("Subject-level parameters not extractable; skipping regression recovery plots.")

# ------------------ save combined summary of posterior comparison ------------------
# Merge fitted and recovered summaries of selected params for side-by-side
def load_summary_table(path):
    return pd.read_csv(path, index_col=0)

fitted_sum_tbl = load_summary_table(FIG_DIR_ROOT / "fitted_summary_rhat_ess.csv")
recovered_sum_tbl = load_summary_table(FIG_DIR_ROOT / "recovered_summary_rhat_ess.csv")
# prefix columns for clarity
fitted_sum_tbl = fitted_sum_tbl.add_prefix("fitted_")
recovered_sum_tbl = recovered_sum_tbl.add_prefix("recovered_")
combined = pd.concat([fitted_sum_tbl, recovered_sum_tbl], axis=1, join="inner")
combined.to_csv(FIG_DIR_ROOT / "combined_parameter_comparison.csv")
print(f"Saved combined summary to {FIG_DIR_ROOT / 'combined_parameter_comparison.csv'}")
...
# recovered_nc = os.path.join(BASE_MODEL_DIR, "mES_29_recovery.nc")
