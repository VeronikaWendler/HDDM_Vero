...
def run_model(trace_id, data, model_dir, model_name, version, phase, samples=4000, accuracy_coding=True): 
    ...
    if phase == 'LE':
        ...
        if version == 0:
            ...
            v_reg = {'model': 'v ~ 1 + z_AttentionW + z_IAW_chart + z_IAW_image', 'link_func': lambda x: x}
            reg_descr = [v_reg]
            depends_on={'t': 'OVcate'} 
        elif version == 30:
...
if __name__ == "__main__":
    ...
    data_full = pd.read_csv((PROJECT_DIR / "data_sets" / "combined_Garcia_OV.csv").as_posix(), sep=",")
    ...
    for phase in PHASE_RUN_ORDER:
        ...
        for version, model_name in enumerate(model_versions[phase]):
            ...
            data = data[~data["subj_idx"].isin({1,4,5,6,14,99,106, 114, 120, 126, 102, 109, 118})]
