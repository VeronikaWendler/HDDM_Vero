%% Clean up the environment - Chih Chung Tings code 
%-----------------------------------------
clear all
clc
close all
...
for k_subj = 1:nSubj
    ...
    beta1 = T.mean(r(4));
    beta2 = T.mean(r(5));
    paramset(k_subj,:) = [a ndt z beta1 beta2 minSigma(k_subj)];
    ...
    paraname_group = [a ndt z beta1 beta2 theta];
end
convergeset = Tconv.Gelman_Rubin;

save('D:/Aberdeen_Uni_June24/cap/THESIS/Garcia_Analysis/stats_TingGluth/Analysis_Simulation_replication/simulation/Sim/Value_mod_withSP/Sim_HDDM_ES_VAL_7_theta3','paramset','convergeset','paraname_group')

% Nrep = 100;

%% Start simulation
load('D:/Aberdeen_Uni_June24/cap/THESIS/Garcia_Analysis/stats_TingGluth/Analysis_Simulation_replication/simulation/Sim/Value_mod_withSP/Sim_HDDM_ES_VAL_7_theta3')
TBsim = [];
...
for k_subj = 1:nSubj
    ...
    for k_OV = 1:3
        ...
        for ktrial = 1:length(data_subj.sub_id)
            ...
            [Choice, RT, E, tempEyeData, FixAaLL, sumdvALL, FixDur] = EvidenceAccumulate_ES_ZBIAS_m26(k_OV, values(ktrial,1), values(ktrial,2), params);
...
save('C:/Cluster_Github/HDDM_Vero/CCT_Matlab_PPC/sim/sim_ES_VAL_7_theta3','TBsim','paramset','subjlist')
close(h)
