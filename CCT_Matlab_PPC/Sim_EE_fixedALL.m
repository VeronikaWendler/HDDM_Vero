...
% Filter only trials where phase is 'EE'
data = data(strcmp(data.phase, 'EE'), :);
...
param_file = ['params_EE_m' num2str(m_num) '.csv'];
converging_file = ['gelman_rubin_EE_m' num2str(m_num) '.csv'];
conv_criteria = 1.1;
...
save('D:/Aberdeen_Uni_June24/cap/THESIS/Garcia_Analysis/stats_TingGluth/Analysis_Simulation_replication/simulation/Sim/with_Fix_weight/Sim_HDDM_EE_paramSim_fixedALL','paramset','convergeset','paraname_group')

%% Start simulation
load('D:/Aberdeen_Uni_June24/cap/THESIS/Garcia_Analysis/stats_TingGluth/Analysis_Simulation_replication/simulation/Sim/with_Fix_weight/Sim_HDDM_EE_paramSim_fixedALL')
TBsim = [];
...
for k_subj = 1:nSubj
    ...
    for k_OV = 1:3
        ...
        data_subj = data(strcmp(data.phase, 'EE') & data.sub_id == subjID & data.OV == k_OV, {'phase', 'sub_id', 'VD', 'p1', 'p2', 'Correct', 'Choice'});
        values = [data_subj.p1, data_subj.p2].*100; % Check this ... Convert values to percentages
        
        for ktrial = 1:length(data_subj.sub_id)
            % Simulate aDDM process
            [Choice, RT, E, tempEyeData, FixAaLL, sumdvALL, FixDur] = EvidenceAccumulate_EE(k_OV, values(ktrial,1), values(ktrial,2), params);
...
save('D:/Aberdeen_Uni_June24/cap/THESIS/Garcia_Analysis/stats_TingGluth/Analysis_Simulation_replication/simulation/Sim/with_Fix_weight/sim_EE_fixedALL','TBsim','paramset','subjlist')
close(h)
