% - Main script used to perform evidence accumulation for each trial - from Chih
% Chung Ting
% 2022/12: add output: DwellFirst, DwellMiddle, DwellFinal, FixLocFirstCorr

function [Choice, RT, E, tempEyeData, FixAaLL, sumdvALL, FixDur]= EvidenceAccumulate_ES_VAL_m2(OV,Vl,Vr,params)


a     = params(1).*0.5;        % boundary seperation 
ndt   = params(2);             % non-decision time
z     = params(3);             % starting point bias - this is only used in models who have it 
beta1 = params(4)./1000;       % intercept  - some models also do not have this param
beta2 = params(5)./1000;       % attention
beta3 = params(6)./1000;       % inattention
noisy = 0.03;%params(6);       %
...
dv       = zeros(pre_Nfix,1)+spbias;  % adding spbias (Chih-Chung suggests adding spbias at the end)
...
while k_fix< pre_Nfix
    ...
    %'model': 'v ~ 1 + V_E + V_S
        if FixR(k_fix) == 1
            ...
            dv(k_fix) = beta1+beta2.*Vl - beta3.*Vr; % for each ms. In HDDM, the smallest unit is s.
        end
%---------------------------------------------------------------------------------------------------------
...
% %---------------------------------------------------------------------------------------------------------
    ...
    err = normrnd(0,noisy,1,FixDur(k_fix)); % Sampled errors for N (FixDur(k_fix)) time point
...
if isempty(ID)
    ...
    FixAaLL      = [Fix.A{1:k_fix}]; % put all lables into the same array
