...
function [Choice, RT, E, tempEyeData, FixAaLL, sumdvALL, FixDur]= EvidenceAccumulate_ES_ZBIAS_m14(OV,Vl,Vr,params,trial_type)

a_ET     = params(1).*0.5;        % boundary seperation 
a_LT  = params(2).*0.5;
ndt   = params(3);             % non-decision time
beta1 = params(4)./10000;       % intercept
beta2 = params(5)./10000;       % attention
beta3 = params(6)./10000;       % inattention
beta4 = params(7)./10000;
beta5 = params(8)./10000;
noisy = 0.03;%params(6);       %


%% pre-determine fixations
pre_Nfix = 70; % assume 70 fixations before decision
FixR     = rand()> 0.69;   % 0.71                0.69256
...
while k_fix< pre_Nfix
    ...
    k_fix = k_fix+1; % Fixation from fixation
     ...
     if strcmp(trial_type, "ET")
        a = a_ET;
        if FixR(k_fix) == 1
            dv(k_fix) = beta1+beta2.*Vr - beta3.*Vl 
        else
            dv(k_fix) = beta1+beta3.*Vr - beta2.*Vl 
        end
...
%         end
     ...
     elseif strcmp(trial_type, "LT")
         a = a_LT;
         if FixR(k_fix) == 1
           dv(k_fix) = beta1+beta4.*Vr - beta5.*Vl; % for each ms. In HDDM, the smallest unit is s.
        else
            dv(k_fix) = beta1+beta5.*Vr - beta4.*Vl; % for each ms. In HDDM, the smallest unit is s.
         end
     end    
        %---------------------------------------------------------------------------------------------------------
